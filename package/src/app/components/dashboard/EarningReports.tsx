      ...
      <div className="flex flex-col mt-2">
        {EarningReportsData.map((item, index) => (
          ...
          >
            ...
            <Badge
              variant="success"
              className={`flex items-center gap-1 bg-lightsuccess text-success`}
            >
              <Icon icon="solar:alt-arrow-up-bold" height={12} />
              {item.statustext}
            </Badge>
          </div>
        ))}
        ...
        <Link
          href={"/"}
          className="text-base text-center pt-6 font-semibold text-dark dark:text-darklink hover:text-primary dark:hover:text-primary"
        >
          View more markets
        </Link>
      </div>
